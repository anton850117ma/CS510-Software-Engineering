...
public class CFGTest {
    ...
    public void reachable_unreachable() {
        ...
        assertFalse(cfg.isReachable(59, m_m, m, 0, m_m, m));
    }

    @Test 
    public void reachable_missingSrc() {

        assertFalse(cfg.isReachable(-100, m_m, m, 0, m_m, m));
    }
    ...
    //below are additional test sets for isReachable
    @Test   
    public void reachable_true2() {

        
        cfg.addEdge(0, m_m, m, 3, m_m, m);  //add an edge from source node to target node
        assertTrue(cfg.isReachable(0, m_m, m, 3, m_m, m));
    }

    @Test
    public void reachable_unreachable2() {

        cfg.addNode(59, m_m, m);            //only add nodes
        cfg.addNode(0, m_m, m);
        assertFalse(cfg.isReachable(59, m_m, m, 0, m_m, m));
    }

    @Test   
    public void reachable_missingSrc2() {

        cfg.addNode(0, m_m, m);             //only add target node to nodes
        assertFalse(cfg.isReachable(-100, m_m, m, 0, m_m, m));
    }

    @Test   
    public void reachable_missingTarget2() {

        cfg.addNode(0, m_m, m);             //only add source node to nodes
        assertFalse(cfg.isReachable(0, m_m, m, 1000, m_m, m));
    } 
